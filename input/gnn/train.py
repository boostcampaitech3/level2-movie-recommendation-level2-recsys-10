...
def main():
    ...
    parser.add_argument("--layers", nargs="+", type=int, default=[64, 64], help=" ")
    ...
    for epoch in range(args.epochs):
        ...
        print(f"[Train] time: {training_time:4.2}s, Loss: {loss:2.5}")
        ...
        if epoch % args.eval_N  == (args.eval_N - 1):
            with torch.no_grad():
                ...
                recall, ndcg = eval_model(
                    ...
                    Rtr=  data_generator.R_train,
                    Rte= data_generator.R_test,
                    k= args.k,
                    device= device
                )
                
            print(f"[Valid] time: {time()-t2:4.2}s, ", end='')
            print(f"Loss: {loss:.4} | Recall@{args.k}: {recall:.4} | NDCG@{args.k}: {ndcg:.4}")
