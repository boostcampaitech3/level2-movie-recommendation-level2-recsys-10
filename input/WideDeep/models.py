...
class WideDeep(nn.Module):
    def __init__(self, input_dims : list, mlp_dims : list, args):
        super(WideDeep, self).__init__()
         # input_dims = [user, item, item_gernes, next, next_gernes] 
        self.total_input_dim = int(sum(input_dims))
        # Wide
        self.wide = Wide(self.total_input_dim, 1)

        # Deep
        self.user_embed = nn.Embedding(
            num_embeddings= input_dims[0], embedding_dim= args.embedding_dim
        )
        self.item_embed = nn.Embedding(
            num_embeddings= input_dims[1], embedding_dim= args.embedding_dim
        )
        self.next_embed = nn.Embedding(
            num_embeddings= input_dims[3] , embedding_dim= args.embedding_dim
        )
        self.genre = nn.Linear(in_features=18, out_features= args.embedding_dim, bias = False)
        self.next_genre = nn.Linear(in_features=18, out_features= args.embedding_dim, bias = False)


        # Linear layer for genres
        self.linaer = nn.Linear(in_features= self.total_input_dim, out_features = args.embedding_dim, bias= False)
        self.global_bias = nn.Parameter(torch.zeros((1,)))
        # embedding matrix
        self.embedding = nn.Embedding(self.total_input_dim+1, args.embedding_dim)
        # mlp_lpayers input dim
        ...
        for i, dim in enumerate(mlp_dims):
            ...
            mlp_layers.append(nn.Dropout(args.hidden_dropout_prob))
    ...
    def deep(self, x):
        # x : (batch_size, user(1) item(1) item_genres(17) next(1) next_genres(18))
        # print("x shape : ", x.shape)
        users, items, i_genres, next, n_genres = x[:,0], x[:, 1], x[:, 2:20].to(torch.float32) , x[:, 20] , x[:, 21:].to(torch.float32)

        embed_users = self.user_embed(users)

        embed_items = self.item_embed(items) # [batch, 1 , embed_dim]
        embed_igeres = self.genre(i_genres)
        embed_next = self.next_embed(next)
        embed_ngeres = self.next_genre(n_genres)

        # embed_x = self.embedding(x) # [50, 3, args.embedding_dim = 10(default)]
        # embed_genre = self.linear(genres).unsqueeze(1) # [50, 1, 10]

        #inputs = embed_x.view(-1, self.embedding_dim)
        inputs = torch.cat([embed_users, embed_items , embed_igeres, embed_next, embed_ngeres], dim = 1).view(-1, self.embedding_dim)
        deep_y = self.mlp_layers(inputs)

        return deep_y

    def forward(self, x):
        # x = [[batch], [user, item, next, item_genres(18), next_genres(18)]] 
