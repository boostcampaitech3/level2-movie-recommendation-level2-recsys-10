...
from models import multivae, recvae, recvae_ract
...
# model parameter
parser.add_argument('--model', type=str, default='RaCTRecVAE', help='model type (default: RaCTRecVAE)')
...
# parser.add_argument("--actor_pretrain_epochs", type=int, default=20, help="number of actor pretrain epochs")
# parser.add_argument("--critic_pretrain_epochs", type=int, default=50, help="number of critic pretrain epochs")
parser.add_argument("--actor_pretrain_epochs", type=int, default=1, help="number of actor pretrain epochs")
parser.add_argument("--critic_pretrain_epochs", type=int, default=1, help="number of critic pretrain epochs")
parser.add_argument("--epochs", type=int, default=50, help="number of finetune epochs")
...
# -- pretrain model path
pre_model_path = os.path.join(save_dir_path, 'pretrained.pth')
...
# # -- model
# model_module = getattr(import_module("models"), args.model)  # default: RaCTRecVAE
# model = model_module(
#     hidden_dim = args.hidden_dim, 
#     latent_dim = args.latent_dim, 
#     input_dim = train_dataset.n_items,
#     dropout_rate = args.dropout_rate,
#     pre_model_path = save_dir_path, 
...
def trainer(model, opts, train_loader, n_epochs, beta=args.beta, gamma=args.gamma):
    ...
    for e in range(n_epochs):
        for batch_index, batch_data in enumerate(train_loader):
            ...
            recon_batch, mu, logvar = model(input_data)
            loss = model.loss_function(recon_batch, input_data, mu, logvar, beta, gamma)
            loss.backward()
...
total_epochs_list= [args.actor_pretrain_epochs, args.critic_pretrain_epochs, args.epochs]
train_stages_list = ['actor_pretrain', 'critic_pretrain', 'finetune']

for epochs, train_stage in zip(total_epochs_list, train_stages_list):
    print('-+-+'*50)
    print('+-+-'*20, f'{train_stage}', '+-+-'*20)
    print('-+-+'*50)
    print()
    # -- model; train stage 각 세 단계마다 모델을 다시 정의해줘야한다. 
    model_module = getattr(import_module("models"), args.model)  # default: RaCTRecVAE
    model = model_module(
        hidden_dim = args.hidden_dim, 
        latent_dim = args.latent_dim, 
        ...
        train_stage = train_stage # iter: ['actor_pretrain', 'critic_pretrain', 'finetune']
    ).to(device)
    ...
    for epoch in range(epochs):
        ...
        running_loss = 0.0
        print()
        epoch_print = f"| [EPOCH: {epoch:3}/{epochs}] |"
        ...
        if epoch % args.eval_N  == (args.eval_N - 1) and train_stage == 'finetune':
            recall_list = []
            recall_list_20 = []
            recall_list_50 = []
            ndcg_list = []
            ...
            with torch.no_grad():
                ...
                for batch_data in valid_loader:
                    ...
                    recon_batch, mu, logvar = model(input_data)
                    loss = model.loss_function(recon_batch, input_data, mu, logvar, gamma=1, beta=None)
            ...
            torch.save(model.state_dict(), f"{save_dir_path}/pretrained.pth")

        print("="*(80+len(epoch_print)))
        if should_stop == True: break


###### ###### ###### ###### ###### [ (1) pretrain actor | train & validation ] ###### ###### ###### ###### ###### 
###### ###### ###### ###### ###### [ (1) pretrain actor | train & validation ] ###### ###### ###### ###### ###### 
###### ###### ###### ###### ###### [ (1) pretrain actor | train & validation ] ###### ###### ###### ###### ###### 
###### ###### ###### ###### ###### [ (1) pretrain actor | train & validation ] ###### ###### ###### ###### ###### 
###### ###### ###### ###### ###### [ (1) pretrain actor | train & validation ] ###### ###### ###### ###### ###### 
...
###### ###### ###### ###### ###### [ train & validation ] ###### ###### ###### ###### ###### 
# best_score = -1
# for epoch in range(args.actor_pretrain_epochs):
#     # -- train
#     running_loss = 0.0
#     print()
