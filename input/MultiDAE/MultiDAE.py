...
from datasets import MultiDAEDataset, MultiDAEValidDataset
...
def submission_multi_dae(args, model):
    DATA_DIR = args.data
...
def main():
    ...
    parser.add_argument("--model_name", default="MultiDAE", type=str)
    ...
    parser.add_argument('--save', type=str, default='Multi_DAE.pt',
    ...
    if args.model_name == 'MultiDAE':
        train_batch_size = 250
        valid_batch_size = 500

        # 만들어준 데이터 셋을 바탕으로 Dataset과 Dataloader를 정의
        train_dataset = MultiDAEDataset()
        valid_dataset = MultiDAEValidDataset(train_dataset = train_dataset)
        ...
        model = MultiDAE(p_dims=[200, 600, n_items]).to(device)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for batch_idx, batch_data in enumerate(train_loader):
                ...
                loss = model.loss_function(recon_batch, input_data)
    ...
    submission_multi_dae(args, model)
